...
maas admin subnet update 8 gateway_ip=10.32.16.1
maas admin subnet update 8 dns_servers=10.32.16.1
maas admin subnet update 1 gateway_ip=
EOF
...
for i in `maas admin machines read | jq '.[] | select(.hostname | contains("cos")) | .system_id '`; do
export SYSTEM_ID=`echo $i | tr -d '"'`;
maas admin interfaces read $SYSTEM_ID | jq '.[] | select(.name | contains("enp6s0")) | .id ' | xargs -i maas admin interface link-subnet $SYSTEM_ID {} mode=AUTO subnet=8;
done
EOF
